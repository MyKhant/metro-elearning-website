<?php
  ...
  function Login(array $data)
  {
    ...
    if (!$Email_check['status']) {
      ...
      if (password_verify($Password, $Email_check['data']['password'])) {
        ...
        header("Location: ../metro-elearning/index.php");
      }
  ...
  */
  ...
  {
    $dbHandler = DbHandler();
    $statement = $dbHandler->prepare("SELECT `user_id`, `first_name`, `last_name`, `email`, `password` FROM `user` WHERE `email` = :email");
    ...
    // Set session login user name
    $_SESSION["first_name"] = $result['first_name'];
    $_SESSION["last_name"] = $result['last_name'];
    $_SESSION["user_id"] = $result['user_id'];
    $_SESSION["user_email"] = $result['email'];
    
    // Set return value for validation of login user's database 
