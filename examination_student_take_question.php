...
<body>
    ...
    <!-- Get Exam type, Subject type and number of Question !--> 
    <?php  
    ...
    <div class="container mt-3">
        <h1>The <span class="text-success"><?php echo $exam_type . " (" . $subject_type . ")"; ?></span> Exam </h1>
        ...
        <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="question-form">
            <div class="question">
                ...
                <?php                     
                    ...
                    $sql = $conn->prepare("SELECT * FROM `tbl_question` WHERE subject = '$subject_type'");
                    $sql->execute();
                    $result = $sql->fetchAll();
                    static $i = 1;
                    foreach ($result as $row) {
                        if ($i > $num_question) break;
    ...
    <!-- Get user's information, Question, answer and insert into the database !-->
    <?php    
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (isset($_POST['submitAnswer'])) {
                ...
                $exam_id = uniqid(); //generate random exam-id string 13 numbers 
                $exam_type = $_POST['exam_type']; // record user name              
                for ($i = 1; $i < $count; $i++) {                    
                    ...
                    try {
                        $sql = "INSERT INTO tbl_temp_question_result (exam_id, exam_type, question_id, subject, student_id, student_name, question, answer_a, answer_b, answer_c, answer_d, correct_answer, check_answer) VALUES ('$exam_id', '$exam_type', '$user_question_id','$user_subject', '$user_id', '$user_name', '$user_question', '$user_answerA', '$user_answerB', '$user_answerC', '$user_answerD', '$user_correctAnswer', '$user_check_answer')";
                        $conn->exec($sql);  
                    ...
                    echo "<script>
                        ...
                        window.location.href = 'http://localhost/metro-elearning/examination_result.php?exam_id=$exam_id&exam_type=$exam_type&subject_type=$user_subject&student_id=$user_id';
                        </script>";
                        exit();
            }                                 
        }
