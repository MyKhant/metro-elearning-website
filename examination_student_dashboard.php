...
<html lang="en">
    ...
    <div class="tab-content" id="nav-tabContent">            
        <!-- Exam Graph Question Dashboard Start -->
				    ...
				    <nav class="navbar navbar-expand-lg navbar-light bg-light">
				        ...
				        <div class="container-fluid p-3"> <!-- Right Side Content Start-->	
					        ...
					        <div class="row mt-4 flex-column flex-lg-row"> <!-- Content Row 2 -->
						        <div class="col">
							        <h2 class="h6 text-white">YOUR PROGRESS</h2>
						            ...
						            <div class="col">
							            <h2 class="h6 text-white">YOUR COURSE AND EXAM DATA</h2>
							            <div class="card mb-3" style="height: 280px">
								            <div class="card-body">
        ...
        <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
            ...
            <div class="table-area mt-3 bg-light">
                <table class="table" style="color: black;">
                    <thead>
                        <tr>
                            <th scope="col" class="col-lg-1 text-center">No.</th>
                            <th scope="col" class="col-lg-2 text-left">Exam</th>
                    ...
                    <tbody>
                        <?php 
                            if ($_SERVER["REQUEST_METHOD"] === "POST") {
                                if (isset($_POST['type_exam'], $_POST['subject_type'], $_POST['result_type'], $_POST['start_date'], $_POST['end_date'])) {
                                    ...
                                    if($type_exam != "All" && $type_subject != "All" && $result_type != "All")
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((exam_type = '$type_exam') && (subject = '$type_subject') && (result = '$result_type') && (student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                                                        
                                    else if($type_exam != "All" && $type_subject != "All" && $result_type == "All") 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((exam_type = '$type_exam' && subject = '$type_subject'  && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                                                        
                                    else if($type_exam != "All" && $type_subject == "All" && $result_type != "All" ) 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((exam_type = '$type_exam' && result = '$result_type'  && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                    
                                    else if($type_exam != "All" && $type_subject == "All" && $result_type == "All" )
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((exam_type = '$type_exam'  && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                    
                                    else if($type_exam == "All" && $type_subject != "All" && $result_type != "All" ) 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((subject = '$type_subject' && result = '$result_type' && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                        
                                    else if($type_exam == "All" && $type_subject != "All" && $result_type == "All" ) 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((subject = '$type_subject'  && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                        
                                    else if($type_exam == "All" && $type_subject == "All" && $result_type != "All" ) 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((result = '$result_type'  && student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");
                                    
                                    else if($type_exam == "All" && $type_subject == "All" && $result_type == "All" ) 
                                        $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE ((student_id = '$user_id') && (date_taken between '$start_date' AND '$end_date'))");                                                                                                         
                                }                                                              
                            }
                            else $stmt = $conn->prepare("SELECT * FROM `tbl_result` WHERE student_id = '$user_id'");
                        ...
                        <?php
                                ...
                                echo "
                                    <script>
                                        ...
                                        window.location.href = 'http://localhost/metro-elearning/examination_student_dashboard.php';
                                    </script>
                                    ";
                                exit();
                            }                    
